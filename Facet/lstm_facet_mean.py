...
def main(unused_args):
    ...
    for key in keys:
        ...
        for index, f in enumerate(grp):
            ...
            if len(data) == 0:
                temp = raw_dt[1:len(raw_dt[:,1]),5:len(raw_dt[1,:])]
                temp2 = []
            ...
            if cntr > limit-(validcnt+testcnt):
                if "S11" in f:
                    extra = 0 if (len(raw_dt[:,1])-1)%5 == 0 else 1
                    for k in range(int(math.floor((len(raw_dt[:,1])-1)/5)+extra)):
                        sp.append("S11")
                elif "S12" in f:
                    extra = 0 if (len(raw_dt[:,1])-1)%5 == 0 else 1
                    for k in range(int(math.floor((len(raw_dt[:,1])-1)/5)+extra)):
            ...
            if len(label) == 0:
                if Y[key+"_"+mat.group(0)] == 0:
                    ...
                    extra = 0 if (len(raw_dt[:,1])-1)%5 == 0 else 1
                    label = np.tile([0],(math.floor((len(raw_dt[:,1])-1)/5)+extra,1))
                else:
                    #label = np.tile([1],(len(raw_dt[:,1])-1,1))
                    extra = 0 if (len(raw_dt[:,1])-1)%5 == 0 else 1
                    label = np.tile([1],(math.floor((len(raw_dt[:,1])-1)/5)+extra,1))
            else:
                if Y[key+"_"+mat.group(0)] == 0:
                    #label = np.concatenate((label,np.tile([0],(len(raw_dt[:,1])-1,1))),axis=0)
                    extra = 0 if (len(raw_dt[:,1])-1)%5 == 0 else 1
                    label = np.concatenate((label,np.tile([0],(math.floor((len(raw_dt[:,1])-1)/5)+extra,1))),axis=0)
                else:
                    #label = np.concatenate((label,np.tile([1],(len(raw_dt[:,1])-1,1))),axis=0)
                    extra = 0 if (len(raw_dt[:,1])-1)%5 == 0 else 1
                    label = np.concatenate((label,np.tile([1],(math.floor((len(raw_dt[:,1])-1)/5)+extra,1))),axis=0)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        with tf.variable_scope("model", reuse=None, initializer=initializer):
          m = FacetModel(is_training=True, config=config)
        with tf.variable_scope("model", reuse=True, initializer=initializer):
          #mvalid = FacetModel(is_training=False, config=config)
          mtest = FacetModel(is_training=False, config=eval_config)
