...
--------------------------------------------------------------------------------------------------------------------------------------------
Instructions to run:
1. lstm_facet_mean_with_cv.py is a python script to perform CV, that runs another script- validator.py multiple times for validation and testing. To run this please change the following variables as necessary:
lines 9-12 of lstm_facet_mean_with_cv:
validation_runs = 51
valid_cnt = 5
test_cnt = 5
limit = 36
...
limit is the total no. of videos to be trained+tested on from the facet directory. 
Also lines 17-23 of the above file can be altered to specify the range of each parameter value to be selected from.

and lines 309-310 of validator.py:
wb = load_workbook('/home/prabhanjan/Downloads/Matlab_MetaData.xlsx', read_only=True)
dir = '/media/prabhanjan/New/DR/Facet Features/'
where wb is the location of the Matlab_Metadata.xlsx and dir is the directory path to facet features (all facet files should be in 1 single directory). 
...
--------------------------------------------------------------------------------------------------------------------------------------------
1. lstm_facet_mean.py is a standalone code that runs lstm on the facet features without doing CV. To run this please change the following variables as necessary:
lines 333-336 (first few lines inside the main() function):
wb = load_workbook('/home/prabhanjan/Downloads/Matlab_MetaData.xlsx', read_only=True)
dir = '/media/prabhanjan/New/DR/Facet Features/'
testcnt = 5
limit = 36
where wb is the location of the Matlab_Metadata.xlsx and dir is the directory path to facet features (all facet files should be in 1 single directory). testcnt is the count of the files to be tested. limit is the no. of videos to be trained+tested on from the facet directory. 
