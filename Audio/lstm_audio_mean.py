...
def main(unused_args):
    ...
    for key in keys:
        ...
        for index, f in enumerate(grp):
            ...
            if len(data) == 0:
                temp = raw_dt[0:len(raw_dt[:,1]),0:len(raw_dt[1,:])]
                temp2 = []
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        with tf.variable_scope("model", reuse=None, initializer=initializer):
          m = AudioModel(is_training=True, config=config)
        with tf.variable_scope("model", reuse=True, initializer=initializer):
          #mvalid = AudioModel(is_training=False, config=config)
          mtest = AudioModel(is_training=False, config=eval_config)
