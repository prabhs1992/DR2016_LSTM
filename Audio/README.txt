...
In the audio_mat_to_txt.m matlab script, replace the line 2 and 4:
    mat_loc = 'E:\DR\AudioFeatures\FeaturesandFormants\FeaturesALL';
    csv_loc = 'E:\DR\NewAudio';
with the location of the FeaturesALL directory inside the audio features folder, and with the new directory where all the processed and new audio features need to be placed.
This script removes the VUV feature=0 samples from the data and also the creaky features(features that have the same value for all samples)

--------------------------------------------------------------------------------------------------------------------------------------------
Instructions to run:
1. lstm_audio_mean_cv.py is a python script to perform CV, that runs another script- validator_audio.py multiple times for validation and testing. To run this please change the following variables as necessary:
lines 9-12 of lstm_audio_mean_with_cv:
validation_runs = 51
valid_cnt = 5
test_cnt = 5
limit = 36
...
limit is the total no. of videos to be trained+tested on from the audio directory. 
Also lines 17-23 of the above file can be altered to specify the range of each parameter value to be selected from.

and lines 309-310 of validator_audio.py:
wb = load_workbook('/home/prabhanjan/Downloads/Matlab_MetaData.xlsx', read_only=True)
dir = '/media/prabhanjan/New/DR/NewAudio/'
where wb is the location of the Matlab_Metadata.xlsx and dir is the directory path to NEW audio features created by the audio_mat_to_txt.m script.
...
--------------------------------------------------------------------------------------------------------------------------------------------
1. lstm_audio_mean.py is a standalone code that runs lstm on the audio features without doing CV. To run this please change the following variables as necessary:
lines 332-335 (first few lines inside the main() function):
wb = load_workbook('/home/prabhanjan/Downloads/Matlab_MetaData.xlsx', read_only=True)
dir = '/media/prabhanjan/New/DR/NewAudio/'
testcnt = 5
limit = 36
where wb is the location of the Matlab_Metadata.xlsx and dir is the directory path to aNEW audio features created by the audio_mat_to_txt.m script.
testcnt is the count of the files to be tested. limit is the no. of videos to be trained+tested on from the audio directory. 
