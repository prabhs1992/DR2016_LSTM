...
for j in range(validation_runs):
    ...
    command = "python ./validator_okao.py --valid_cnt "+str(valid_cnt)+" --test_cnt "+str(test_cnt)+" --limit "+str(limit)+" --vt "+str(1)+" --init_scale "+str(init_scale)+" --max_grad_norm "+str(max_grad_norm)+" --max_epoch "+str(max_epoch)+" --max_max_epoch "+str(max_max_epoch)+" --keep_prob "+str(keep_prob)+" --lr_decay "+str(lr_decay)+" --num_layers "+str(num_layers)
    os.system(command)
    import time
    time.sleep(2)
